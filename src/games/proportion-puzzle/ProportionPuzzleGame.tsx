...
export function ProportionPuzzleGame() {
  ...
  const [problem, setProblem] = useState<ProportionProblem | null>(null)
  ...
  const spawnProblem = useCallback((count: number) => {
    const level = getProportionLevel(count)
    setProblem(generateProportionProblem(level))
    setInput("")
    setFeedback(null)
  ...
  const handleSubmit = useCallback(() => {
    ...
    const correct = checkProportionAnswer(input, problem)
    ...
    <div className="flex flex-col items-center gap-8 w-full max-w-md">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={`w-full rounded-2xl border-2 bg-card px-10 py-10 transition-colors ${
            feedback === "correct" ? "border-success" : feedback === "wrong" ? "border-destructive" : "border-border"
          }`}
        >
          <p className="text-xs text-muted-foreground text-center mb-4">Solve for the missing value</p>
          {/* Visual proportion: two stacked fractions with = */}
          <div className="flex items-center justify-center gap-8">
            {/* Left fraction */}
            <div className="flex flex-col items-center gap-1">
              <span className="font-mono text-5xl font-black text-foreground">{problem.a}</span>
              <div className="w-full h-1 rounded-full bg-foreground" />
              <span className="font-mono text-5xl font-black text-foreground">{problem.b}</span>
            </div>
            <span className="text-4xl font-bold text-muted-foreground">=</span>
            {/* Right fraction */}
            <div className="flex flex-col items-center gap-1">
              <span className="font-mono text-5xl font-black text-foreground">{problem.c}</span>
              <div className="w-full h-1 rounded-full bg-foreground" />
              <span className={`font-mono text-5xl font-black ${feedback === "correct" ? "text-success" : "text-primary"}`}>
      ...
      <p className="text-xs text-muted-foreground">Find the missing denominator · Enter to submit</p>
